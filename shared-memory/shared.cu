...
__global__ void nextGeneration(int *current, int *next, int rows, int cols, int *change_flag) {
    ...
    // Copy current grid to shared memory
    shared_mem[local_y * (blockDim.x + 2) + local_x] = current[y * (cols + 2) + x];

    // Copy ghost cells into shared memory
    if (threadIdx.x == 0) shared_mem[local_y * (blockDim.x + 2)] = current[y * (cols + 2) + (x - 1)];
    if (threadIdx.x == blockDim.x - 1) shared_mem[local_y * (blockDim.x + 2) + local_x + 1] = current[y * (cols + 2) + (x + 1)];
    if (threadIdx.y == 0) shared_mem[local_x] = current[(y - 1) * (cols + 2) + x];
    if (threadIdx.y == blockDim.y - 1) shared_mem[(local_y + 1) * (blockDim.x + 2) + local_x] = current[(y + 1) * (cols + 2) + x];

    __syncthreads();

    int neighbors = getNeighborCount(shared_mem, local_x, local_y, blockDim.x + 2, blockDim.y + 2);  // Adjust for shared memory
    int idx = y * (cols + 2) + x;
...
int main() {
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        nextGeneration<<<numBlocks, threadsPerBlock, (THREADS_PER_BLOCK+2)*(THREADS_PER_BLOCK+2)*sizeof(int)>>>(d_current, d_next, rows, cols, d_change_flag);
